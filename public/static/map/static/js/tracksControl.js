 ...
 * @Last Modified time: 2015-11-05 19:50:00
 */
...
var tracksControl = function() {
    this.trace_id = 'WqEEVSF8TvNG4BXalw9Q97pSMmEvaNNz'; //当前trace服务id
    this.ak = '114016'; //当前trace服务关联的ak
    ...
    this.POIS_MAX_NUM = 5000; //是否抽稀的阀值，如果track的POI数超过该值，就进行抽稀
    ...
    this.tmplTheads = {}; //监控面板 表头渲染对象
    this.tracksListTmplObj = {};
    this.monitorTableTmplObj = {}; //监控面板模板对象
    this.historyTableTmplObj = {};
    this.noTracksIds = []; //未查询到轨迹点的track id数组
    this.trackPoiArrayObj = {}; //实时监控中保存实时点近期内的poi点 最大size  100

    this.intervarFlagObj = {}; //实时监控中 marker移动动画的定时器对象

    this.version = 2;
    this.versionPageIndex = {
        "V1":1,
        "V2":1
    }
    /**
    ...
    this.checkServiceVersion = function(){
        var self = this;
        if (typeof self.trace_id === 'undefined' || self.trace_id.length <= 0) {
            ...
            $.get('api/checkVersion.php?time=' + timeStamp + '&traceId=' + self.trace_id, function (data) {
                data = $.parseJSON(data);
                if (data && data.both) {
                    $('.tracks-list-title').css('padding-top', '20px');
                    $('#chk_all_track').css('top', '36px');
                    $('#tracks-list-panel').css('margin-top', '78px');
                    $('#myAlert').append('<p>开发者您好，由于鹰眼服务升级，检测到您的服务存在不同版本的数据，您可以在左侧面板切换服务版本查看数据！</p>');
                    $('.version-tabs').delegate('a', 'click', function (e) {
                        var target = e.target;
                        self.version = $(target).attr('data-version');
                        $('.version-tabs a').removeClass('active');
                        $(target).addClass('active');
                        if (self.version === 1) {
                            self.loadTracks(self.versionPageIndex.V1);
                        } else {
                            self.loadTracks(self.versionPageIndex.V2);
                        }
                    });
                    $('.version-tabs').show();
                } else {
                ...
                if (data.error) {
					/* global jAlert */
				    jAlert('未查询到相关的轨迹数据', '系统提示', function () {
					    return false;
					});
                } else {
                    self.loadTracks(1);
        ...
        $('#tracks-history').bind('click', function() {
            ...
            if (typeof startTime != 'undefined' && typeof endTime != 'undefined' && startTime != 0 && endTime != 0) {
                ...
                if (selected.length <= 0) {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: 'api/trackHistory.php?time=' + timeStamp + '&traceId=' + self.trace_id + '&ak=' + self.ak + '&ids=' + track_ids + '&start_time=' + startTime + '&end_time=' + endTime,
    ...
    this.getTraceDetail = function() {
        ...
        if (typeof self.trace_id === 'undefined' || self.trace_id.length <= 0) {
			...
			var timeStamp = new Date();
			timeStamp = timeStamp.getTime();
            $.get('api/traceDetail.php?time=' + timeStamp + '&traceId=' + self.trace_id + '&ak=' + self.ak, function (data) {
                data = $.parseJSON(data);
                if (data && data.service) {
                    self.trace = data.service;
                    $('.trace-admin-title').html('(车辆位置管理平台' + '<span class="trace-admin-circle"></span>' + '<span style="color:#ffffff">' + self.trace.name + '</span>' + ')');
                    self.tmplTheadObj = {};
    ...
    this.loadTracks = function(pageIndex) {
        ...
        if(self.version==1){
            self.versionPageIndex.V1=pageIndex;
        }else{
            self.versionPageIndex.V2=pageIndex;
        }
        $('.tracks-loading').show();
        var timeStamp = new Date();
		timeStamp = timeStamp.getTime();
		var url = 'api/tracksList.php?time=' + timeStamp + '&traceId=' + self.trace_id + '&ak=' + self.ak + '&pageIndex=' + pageIndex + '&pageSize=' + self.pageSize;
    ...
    this.tracksListTableRender = function(pageIndex, pageSize) {
        ...
        if (self.tracksList.length <= 0) {
            ...
            if (pageNums > 1) {
                ...
                var width = $('#tracks-pager-ul').width();
                if(width >200 ){
                     $('#tracks-pager-ul').css('padding-left','2px');
                }else{
                    $('#tracks-pager-ul').css('padding-left','18px');
                }
            } else{
                $('#tracks-pager-ul').hide();
            }
    ...
    this.monitorTrackById = function(track_id, setFiew) {
        ...
        if (self.tracksMonitoring.length > 0) {
			var timeStamp = new Date();
			timeStamp = timeStamp.getTime();
            $.get('api/trackDetail.php?time=' + timeStamp + '&traceId=' + self.trace_id + '&ak=' + self.ak + '&track_id=' + track_id + '&version=' + self.version, function (data) {
                data = $.parseJSON(data);
                if (data && data.status == 0 && data.poi != null) {
                    for (var i = 0; i < self.tracksMonitoring.length; i++) {
                        if (self.tracksMonitoring[i].track_id == track_id) {
                            self.tracksMonitoring[i] = data.poi;
                        }
                    }
                    var l = self.trackPoiArrayObj[track_id] ? self.trackPoiArrayObj[track_id].length : 0,
                        pt = new BMap.Point(data.poi.location[0], data.poi.location[1]),
                    ...
                    var marker = self.createMarker(data.poi);
                    if (l > 1) {
                    ...
                    if (setFiew) {
                        ...
                        $('#loctime-' + track_id).slideUp('normal').html(self.js_date_time(data.poi.loc_time)).slideDown('normal');
    ...
    /**
     * [getRotation 计算小车marker的旋转角度]
     * @Author: xuguanlong
     * @param   {[type]}   curPos    [当前point]
     * @param   {[type]}   targetPos [目标point]
     * @return  {[type]}             [旋转角度值]
     */
    ...
    /**
     * [createMarker 创建marker]
     * @Author: xuguanlong
     * @param   {[type]}   poi [实时poi点]
     * @return  {[type]}       [description]
     */
    this.createMarker = function(poi) {
        ...
        if (self.trackPoiArrayObj[poi.track_id].length > 3) {
            var l = self.trackPoiArrayObj[poi.track_id].length;
            deg = self.getRotation(self.trackPoiArrayObj[poi.track_id][l - 4], self.trackPoiArrayObj[poi.track_id][l - 1]);
        }
     ...
     * @Author: xuguanlong
     * @param   {[type]}   unixtime1 [经纬度坐标点1的loc_time]
     * @param   {[type]}   unixtime2 [经纬度坐标点2的loc_time]
    ...
    this.encrytLngLatpois = function(unixtime1, unixtime2, p1, p2) {
        ...
        for (var i = 1; i < count; i++) {
            ...
            pos = self.projection.pointToLngLat(new BMap.Pixel(x, y));
            //将loc_time添加到point对象中 播放过程中方便于实时读取时间
    ...
    /**
     * [highLight 鼠标hover轨迹上时联动面板中轨迹tr变色]
     * @Author: xuguanlong
    ...
    /**
     * [showAll 显示全部 map重新设置viewport]
     * @Author: xuguanlong
     * @return  {[type]}   [判断显示类别 如果显示所有marker为1  显示所有轨迹为2]
     */
    ...
    /**
     * [setFitViewForMarker 显示某一个marker  viewPort适合这个marker]
     * @Author: xuguanlong
     * @param   {[type]}   id   [marker id]
     */
    this.setFitViewForMarker = function(id) {
        var self = this,
            lngLats = [];
        if (!$('#monitoring').hasClass('active')) {
            return;
    ...
    /**
     * [setFitViewForTrack 显示某一个track  viewPort适合这个track]
     * @Author: xuguanlong
     * @param   {[type]}   id [track id]
     */
    ...
    /**
     * [toDecimal3 取number 小数点后3位]
     * @Author: xuguanlong
     * @param   {[type]}   num [description]
     * @return  {[type]}       [保留小数点后3位的num]
     */
    ...
    /**
     * [setPageSize 自适应左侧列表的高度 然后根据这个高度重新设定每页显示的track数目]
     * @Author: xuguanlong
     */
    ...
    /**
     * [loopMonitoring 获取所有监控track的实时点]
     * @Author: xuguanlong
     * @return  {[type]}   [description]
     */
    ...
    /**
     * [setTimeControl 轨迹播放时时间显示控件的更新]
     * @Author: xuguanlong
     * @param   {[type]}   str [实时时间的字符串]
     */
    ...
    /**
     * [autoLoadHistory 自动查询所有track的历史轨迹点]
     * @Author: xuguanlong
     * @param   {[type]}   loadAll [是否查询所有的监控track]
     * @return  {[type]}           [description]
     */
    ...
    /**
     * [getTrackHistory 单条track的历史轨迹查询]
     * @Author: xuguanlong
     * @param   {[type]}   track_id [单条track id]
     * @return  {[type]}            [description]
     */
    this.getTrackHistory = function(track_id) {
        ...
        $.ajax({
            type: 'GET',
            url: 'api/trackHistory.php?time=' + timeStamp + '&traceId=' + self.trace_id + '&ak=' + self.ak + '&ids=' + track_id + '&start_time=' + startTime + '&end_time=' + endTime + '&version=' + self.version,
            success: historyCallBack
        });
    }

    /**
     * [inMonitoring  根据track_id  判断是否处于监控中 为了防止请求时间过长 用户删除了轨迹  historyCallBack回调函数无法查到track]
     * @Author: xuguanlong
